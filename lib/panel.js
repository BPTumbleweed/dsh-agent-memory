...
const DATA_KINDS = [
  ...
  { id: 'preferences', title: '偏好档案 preferences/merlin.md', file: (c) => path.join(c.storeRoot, 'preferences', 'merlin.md'), format: 'text' },
...
export function attachPanel(ctx, status, config) {
  ...
  try {
    ...
    register({
      kind: 'exact',
      path: `${prefix}/panel`,
      handler: (req, res) => send(res, 200, 'text/html; charset=utf-8', HTML(prefix, config.storeRoot)),
    })
    status.cap('panel', 'ok',
