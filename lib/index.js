...
import { attachSessionMemory } from './session-memory.js'
...
export default {
  ...
  apply(ctx, rawConfig = {}) {
    ...
    const config = {
      ...
      sessionInjectMax: rawConfig.sessionInjectMax || 2048,
    ...
    // 会话级记忆注入：全局层仍由官方 agent-instructions 负责，这里只注入"仅本会话有效"的部分
    try {
      attachSessionMemory(ctx, status, config)
    } catch (err) {
      status.recordError('sessionMemory', err)
      status.cap('sessionMemory', 'unavailable', String(err?.message ?? err))
    }

    status.cap('tools', 'pending', 'P1 未实现（当前用 memory-note.py 等 CLI）')
    status.cap('injectFallback', 'disabled', '全局注入由官方 agent-instructions 负责；本插件的会话注入见 sessionMemory')
